...
public:
    ...
    std_shared::shared_ptr<TDistributedItem> CreateObject();
    std_shared::shared_ptr<TDistributedItem> CreateObject(const t_DistibutedId&);
    
    virtual int Serialize(ISerializer&);

private:
	typedef std::list<std_shared::weak_ptr<TDistributedItem> > t_objects;
